...
const Roms = () => {
  ...
  const [KernalData, setKernalData] = useState([].slice(10));
  ...
  const getUsers = async () => {
    try {
        ...
        setKernalData(await response.json());
    } catch (error) {
...
useEffect(() => {
  ...
  const displayUsers = KernalData
    .slice(pagesVisited, pagesVisited + usersPerPage)
    .map((KernalData) => {
      const { id, name, link, build ,Device,Dev } = KernalData;
      return (
        ...
        <div className="card ">
          <div className="card-body">
            ...
            <div className="container mx-auto mt-20 grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-1" key={id} >
              ...
              <span className="card-number card-circle subtle">{id}</span>
                ...
                <div className="flex-grow text-white">
                  ...
                  <h3 className="Ver" >{Device}</h3>
                  <h3 className="Ver1" >{build}</h3>
                  <h3 className="Ver1" >{Dev}</h3>
            ...
            <a className="d-btn1 " href="">Discussion</a>
  ...
  const pageCount = Math.ceil(KernalData.length / usersPerPage);
  ...
  const changePage = ({ selected }) => {
    ...
    return (
        ...
        <div className=" Main2 team-1 py-4 md:py-12 bg-gray-100">
          <div className="heading"><h1 className="heading-h1">Kernal Download</h1></div>
